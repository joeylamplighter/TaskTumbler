...
(function() {
    function initializeSyncComponents() {
        ...
        if (typeof window.React === 'undefined' || typeof window.React.useState !== 'function') {
            console.warn('⚠️ React not available - sync components cannot initialize');
            return;
        }
        ...
        function SyncButton({ user, syncState, onOpenModal }) {
            ...
            // Helper to safely check Firebase configuration
            const checkFirebaseConfigured = () => {
                try {
                    ...
                    console.warn('Error checking Firebase config:', e);
                }
                return false;
            };
            ...
            if (!checkFirebaseConfigured() || !user) {
                return (
                    <button 
        ...
        function SyncModal({ user, syncState, lastSyncTime, onClose, onSignIn, onSignOut, onSyncNow, onPullFromCloud }) {
            ...
            const handleEmailSubmit = async (e) => {
                ...
                let firebase = null;
                try {
                    firebase = window.firebase || (typeof firebase !== 'undefined' ? firebase : null);
                } catch (e) {
            ...
            const handlePasswordReset = async () => {
                ...
                try {
                    ...
                    if (!firebase || !firebase.auth) {
                        setMessage({ type: 'error', text: 'Firebase is not available.' });
    ...
    if (typeof window.React !== 'undefined' && typeof window.React.useState === 'function') {
        ...
        setTimeout(() => {
            clearInterval(checkReact);
            if (typeof window.SyncButton === 'undefined') {
                console.error('❌ Sync components failed to load - React not available after 5 seconds');
            }
        }, 5000);
    }
})();
