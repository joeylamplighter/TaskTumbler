...
// ===========================================
// SETTINGS TAB (CLEANED + CATS UI POLISH + NEGATIVE XP + COLLAPSIBLE EDITOR SECTIONS)
// Updated: 2025-12-22
// Notes:
// - Cats editor panels (Rename, Score/Multiplier, Subcategories) are collapsible per-category
// - Collapse state is stored in settings.catsUi (local only, safe to sync if you want)
// ===========================================
...
const [foldSubs, setFoldSubs] = useState(true); // usually the only one you need open
    ...
    const normalizeView = useCallback((view) => {
      ...
      const normalized = mapping[view.toLowerCase()];
      return normalized || "view";
    }, []);

    const [settingsView, setSettingsView] = useState(() => {
      try {
        return normalizeView(initialView) || "view";
      } catch (err) {
        console.error('Error initializing settings view:', err);
        return "view";
      }
    });

    // Game tab collapsible sections state
    ...
    // Locations Manager state (moved to top level to follow Rules of Hooks)
    const [allLocations, setAllLocations] = useState(() => {
      try {
        if (typeof window !== 'undefined' && typeof window.getSavedLocationsV1 === "function") {
    ...
    useEffect(() => {
      ...
      const loadLocations = () => {
        try {
          if (typeof window !== 'undefined' && typeof window.getSavedLocationsV1 === "function") {
            setAllLocations(window.getSavedLocationsV1() || []);
          } else {
            setAllLocations(JSON.parse(localStorage.getItem("savedLocations_v1") || "[]"));
          }
        } catch {
          setAllLocations([]);
        }
    ...
    const updateLocationsGlobally = useCallback((newList) => {
      const next = Array.isArray(newList) ? newList : [];
      setAllLocations(next);
      if (typeof window !== 'undefined' && typeof window.setSavedLocationsV1 === "function") {
      ...
      <div className="fade-in" style={{ paddingBottom: 20 }}>
        <style>{`
          ...
          @media (max-width: 600px) {
            ...
            /* Expanded category takes full width on mobile */
            .ttCatsChipGrid > div[data-category-card] {
          ...
          /* Mobile improvements for expanded category editor */
          @media (max-width: 767px) {
            .ttCatsChipGrid {
              max-height: calc(100vh - 250px) !important;
            }
            
            /* Better spacing for expanded content on mobile */
            [data-expanded-content] {
              padding: 8px !important;
            }
            
            [data-expanded-content] > div:first-child {
              padding: 8px 10px !important;
              font-size: 13px !important;
            }
            
            [data-expanded-content] > div:last-child {
              padding: 10px !important;
              gap: 10px !important;
            }
            
            /* Make inputs more touch-friendly on mobile */
            [data-expanded-content] input,
            [data-expanded-content] button {
              min-height: 40px !important;
              font-size: 14px !important;
            }
            
            /* Better button spacing on mobile */
            [data-expanded-content] .f-input {
...
@media (max-width: 720px){
  ...
  /* Better mobile spacing for category header */
  .ttCatsHeader {
  ...
  /* Make category chips more touch-friendly */
  .ttCatsChipGrid > div[data-category-card] {
    min-height: 70px !important;
  }
  
  /* Better spacing in expanded editor on mobile */
  [data-expanded-content] > div:last-child > div {
    padding: 8px !important;
  }
  
  /* Improve subcategory chips on mobile */
  [data-expanded-content] .ttCatsSubChip {
    padding: 8px 10px !important;
    font-size: 13px !important;
  }
}
        ...
        <div className="segmented-control tt-seg">
          <button className={`sc-btn ${settingsView === "view" ? "active" : ""}`} onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
          ...
          <button className={`sc-btn ${settingsView === "logic" ? "active" : ""}`} onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            setSettingsView("logic");
            try {
              const hash = window.location.hash;
              if (hash.includes('#settings')) {
                const hashParts = hash.split('?');
          ...
          <button className={`sc-btn ${settingsView === "game" ? "active" : ""}`} onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            setSettingsView("game");
            try {
              const hash = window.location.hash;
              if (hash.includes('#settings')) {
                const hashParts = hash.split('?');
          ...
          <button 
            className={`sc-btn ${settingsView === "cats" ? "active" : ""}`} 
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              setSettingsView("cats");
          ...
          <button className={`sc-btn ${settingsView === "data" ? "active" : ""}`} onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
        ...
        {settingsView === "game" && (
          <div className="fade-in-up">
            <style>{`
              ...
              .game-section-chevron.collapsed {
            ...
            <div style={{ marginBottom: 20 }}>
              <h3 style={{ fontFamily: "Fredoka", fontSize: 20, marginBottom: 8, fontWeight: 900, color: "var(--text)" }}>⚔️ Gamification Settings</h3>
              <p style={{ fontSize: 12, color: "var(--text-light)", opacity: 0.7, margin: 0 }}>
                Customize game mechanics, rewards, and feedback systems
              </p>
            </div>
            ...
            <div className="game-section">
              <div 
                className={`game-section-header ${!expandedSections.xp ? 'game-section-header-collapsed' : ''}`}
              ...
              >
                ...
                <span className={`game-section-chevron ${!expandedSections.xp ? 'collapsed' : ''}`} aria-label={expandedSections.xp ? 'Collapse section' : 'Expand section'}>▼</span>
              </div>
              <div className={`game-section-body ${!expandedSections.xp ? 'collapsed' : ''}`}>
            ...
            <div className="game-section">
              <div 
                className={`game-section-header ${!expandedSections.spin ? 'game-section-header-collapsed' : ''}`}
              ...
              >
                ...
                <span className={`game-section-chevron ${!expandedSections.spin ? 'collapsed' : ''}`} aria-label={expandedSections.spin ? 'Collapse section' : 'Expand section'}>▼</span>
              </div>
              <div className={`game-section-body ${!expandedSections.spin ? 'collapsed' : ''}`}>
            ...
            <div className="game-section">
              <div 
                className={`game-section-header ${!expandedSections.duel ? 'game-section-header-collapsed' : ''}`}
                onClick={() => toggleSection('duel')}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') {
              ...
              >
                ...
                <span className={`game-section-chevron ${!expandedSections.duel ? 'collapsed' : ''}`} aria-label={expandedSections.duel ? 'Collapse section' : 'Expand section'}>▼</span>
              </div>
              <div className={`game-section-body ${!expandedSections.duel ? 'collapsed' : ''}`}>
            ...
            <div className="game-section">
              <div 
                className={`game-section-header ${!expandedSections.feedback ? 'game-section-header-collapsed' : ''}`}
              ...
              >
                ...
                <span className={`game-section-chevron ${!expandedSections.feedback ? 'collapsed' : ''}`} aria-label={expandedSections.feedback ? 'Collapse section' : 'Expand section'}>▼</span>
              </div>
              <div className={`game-section-body ${!expandedSections.feedback ? 'collapsed' : ''}`}>
              ...
              <div className="fade-in-up">
                <div className="ttCatsPanel" style={{ borderRadius: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
                  ...
                  <div 
                    ...
                    onClick={(e) => {
                      // Close expanded category only if clicking directly on the grid background
                      // (not on any category card or expanded content)
                      const clickedCard = e.target.closest('[data-category-card]');
                      const clickedExpanded = e.target.closest('[data-expanded-content]');
                      
                      // Only close if we clicked directly on the grid container (not on any card)
                      if (e.target === e.currentTarget && !clickedCard && !clickedExpanded) {
                        setExpandedCat(null);
                      }
                    }}
                  >
                          ...
                          catsFiltered.map((cat) => {
                            ...
                            return (
                              <div
                                ...
                                data-category-card
                                onClick={(e) => {
                                  // Only handle clicks on the card container itself, not children
                                  // Children will handle their own clicks with stopPropagation
        ...
        {settingsView === "data" && (
          <div className="fade-in-up">
            ...
            {typeof window !== 'undefined' && window.LocationsManager && (() => {
              const LocationsManager = window.LocationsManager;
              if (!LocationsManager) return null;
              
              return (
                <div style={{ background: "var(--card)", padding: 16, borderRadius: 12, marginBottom: 24 }}>
                  <h4 style={{ fontFamily: "Fredoka", fontSize: 16, marginBottom: 12 }}>📍 Saved Locations</h4>
                  <LocationsManager 
  ...
  console.log("✅ 13-07-settings.jsx loaded (Cats editor: collapsible cards for Rename, Score/Mult, Subcategories)");
})();
