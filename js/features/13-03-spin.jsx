...
(function () {
  ...
  // -------------------------------------------
  // External UI modules (split files)
  // -------------------------------------------
  const SpinUI = window.SpinUI || {};
  const WinnerPopup = window.SpinWinnerPopup;

  const DateFilterButton = SpinUI.DateFilterButton;
  const DurationFilterButton = SpinUI.DurationFilterButton;
  const MultiSelectButton = SpinUI.MultiSelectButton;
  const QuickCatDongle = SpinUI.QuickCatDongle;

  if (!DateFilterButton || !DurationFilterButton || !MultiSelectButton || !QuickCatDongle) {
    console.warn(
      "⚠️ SpinUI not loaded. Script order must be: 13-03a-spin-ui.jsx, 13-03b-spin-winner-popup.jsx, 13-03-spin.jsx"
    );
  }
  if (!WinnerPopup) {
    console.warn(
      "⚠️ SpinWinnerPopup not loaded. Script order must be: 13-03a-spin-ui.jsx, 13-03b-spin-winner-popup.jsx, 13-03-spin.jsx"
    );
  }

  // -------------------------------------------
  // Category XP + Multiplier helpers (use shared utilities)
  // -------------------------------------------
  const TabUtils = window.TabUtils || {};
  const getCatXpAdjust = TabUtils.getCatXpAdjust || ((cat, settings) => {
    const v = settings?.categoryXpAdjust?.[cat];
    const n = parseInt(v, 10);
    return Number.isFinite(n) ? n : 0;
  });
  const getCatMultiplier = TabUtils.getCatMultiplier || ((cat, settings) => {
    const v = settings?.categoryMultipliers?.[cat];
    const n = Number(v);
    return Number.isFinite(n) ? n : 1;
  });
  const isSpinEligibleCategory = TabUtils.isSpinEligibleCategory || ((cat, settings) => {
  ...
  // ---------- HELPERS (use shared utilities) ----------
  const clamp = TabUtils.clamp || ((n, a, b) => Math.max(a, Math.min(b, n)));
  const dayStart = TabUtils.dayStart || ((d) => new Date(d.getFullYear(), d.getMonth(), d.getDate()));
  const isValidDate = TabUtils.isValidDate || ((d) => d instanceof Date && !isNaN(d.getTime()));
  const parseDueDate = TabUtils.parseDueDate || ((t) => {
    try {
      if (!t) return null;
  ...
  const dueMatches = TabUtils.dueMatches || ((task, filterDue) => {
  ...
  const computeWeight = TabUtils.computeWeight || ((t, settings, useWeighted = true) => {
  ...
  const pickWeightedIndex = TabUtils.pickWeightedIndex || ((items, weights) => {
  ...
  // MAIN COMPONENT
  // ===========================================
  window.SpinTab = function SpinTab({
    tasks,
    categories,
    onView,
    onFocus,
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {showDetailedFilters && ReactDOM.createPortal(
          <div 
  ...
  console.log("✅ 13-03-spin.jsx Loaded (Split Core)");
})();
