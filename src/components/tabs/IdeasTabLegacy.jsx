...
export default function IdeasTabLegacy({ text, setText, onAddTasks, settings, notify, savedNotes, setSavedNotes }) {
    ...
    const handleAiBrainstorm = async () => {
        ...
        const prompt = window.getBrainstormPrompt ? window.getBrainstormPrompt(text) : `Generate creative ideas based on: ${text}`;
    ...
    const saveNewNote = () => {
        ...
        const generateId = window.generateId || (() => 'note_' + Date.now() + '_' + Math.random().toString(36).slice(2));
                ...
                <div className="modal-overlay" onClick={()=>setShowLoadModal(false)}>
                    <div className="modal" onClick={e=>e.stopPropagation()}>
                        ...
                        <div style={{maxHeight:'50vh', overflowY:'auto'}}>
                            {(!savedNotes || savedNotes.length === 0) && <div style={{textAlign:'center', color:'var(--text-light)', padding:20}}>No saved notes found.</div>}
                            {(savedNotes || []).map(note => (
...
if (typeof window !== 'undefined') {
  window.IdeasTab = IdeasTabLegacy;
}
