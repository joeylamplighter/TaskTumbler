...
// --- TASKS TAB MAIN ---
export default function TasksTabLegacy({ tasks, onView, onComplete, onDelete, categories, onAdd, openAdd, notify, settings }) {
  ...
  const displayTasks = useMemo(() => {
      let filtered = sourceList.filter(t => {
          ...
          if (filterTime !== 'Any') {
              ...
              if (filterTime === 'Overdue' && diff >= 0) return false;
              if (filterTime === 'Now' && diff !== 0) return false;
              if (filterTime === 'Tomorrow' && diff !== 1) return false;
              if (filterTime === 'Week' && (diff < 0 || diff > 7)) return false;
              if (filterTime === 'Month' && (d.getMonth() !== today.getMonth() || d.getFullYear() !== today.getFullYear())) return false;
          }
...
// Keep window assignment for backward compatibility
if (typeof window !== 'undefined') {
  window.TasksTab = TasksTabLegacy;
}
